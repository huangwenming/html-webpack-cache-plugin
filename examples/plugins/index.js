    ...
    generateLocalForage() {
        const localForage = function () {
            ...
            window._htmlCacheUtil.cacheConfigure = function (options) {
                ...
                console.log('缓存配置：', options);
                window._htmlCacheUtil.cacheConf = {
                    // 缓存方式，支持localstorage和indexedDB
            ...
            window._htmlCacheUtil.openDataBase = function (dbConf) {
                var request = window._htmlCacheUtil.getIDB().open(dbConf.dbName, dbConf.version);
                request.addEventListener('upgradeneeded', function (event) {
                    var db = event.target.result;
                    var store = db.createObjectStore(dbConf.storeName, {keyPath: dbConf.storeKey});
            ...
            window._htmlCacheUtil.setItem = function (key, value) {
                ...
                function lsSetItem(key, value) {
                    ...
                    console.log('localstorage: 添加数据成功', key);
                }
                // 使用indexedDB
                if (conf.type === 'indexedDB') {
                    var request = window._htmlCacheUtil.openDataBase(conf.dbConf);
                    // 链接数据库成功
                    if (request) {
                        request.addEventListener('success', function (event) {
                            // 获取事务 && store
                            var db = event.target.result;
                            var transaction = db.transaction(conf.dbConf.storeName, 'readwrite');
                            var objectStore = transaction.objectStore(conf.dbConf.storeName);
                            ...
                            addRequest.addEventListener('success', function (event) {
                                console.log('indexedDB: 添加数据成功', key);
                            });
                        });
                    }
                    else {
            ...
            window._htmlCacheUtil.getItem = function (key, successCb, failCb) {
                ...
                function lsGetItem(key) {
                    ...
                    if (cache) {
                        ...
                        console.log('localstorage: 查询数据成功', key);
                    }
                    else {
                        console.log('localstorage: 查询数据为空', key);
                        failCb && failCb();
                    }
                    ...
                    if (request) {
                        request.addEventListener('success', function (event) {
                            ...
                            getRequest.addEventListener('success', function (event) {
                                ...
                                if (cache) {
                                    console.log('indexedDB: 查询数据成功', key);
                                    successCb && successCb(event.target.result.key);
                                }
                                else {
                                    failCb && failCb();
                                }
                            ...
                            getRequest.addEventListener('error', function (event) {
                                console.log('indexedDB: 查询数失败', key);
                                failCb && failCb();
                            });
                        });
                    }
            ...
            window._htmlCacheUtil.removeItem = function (key) {
                ...
                function lsRemoveItem(key) {
                    ...
                    console.log('localstorage: 删除数据成功', key);
                }
                // 使用indexedDB
                if (conf.type === 'indexedDB') {
                    var request = window._htmlCacheUtil.openDataBase(conf.dbConf);
                    ...
                    if (request) {
                        request.addEventListener('success', function (event) {
                            ...
                            deleteRequest.addEventListener('success', function (event) {
                                console.log('indexedDB: 删除数据成功', key);
                            });
                        });
                    }
                    else {
